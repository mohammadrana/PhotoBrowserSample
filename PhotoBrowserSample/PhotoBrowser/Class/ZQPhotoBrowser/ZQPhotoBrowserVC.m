...
//    gradient.colors = [NSArray arrayWithObjects:(id)[[UIColor blackColor] CGColor], (id)[[UIColor blackColor] CGColor], nil];
...
-(void) setupUI {
    ...
    UILabel *topColor = [[UILabel alloc] initWithFrame:CGRectMake(0, 0, self.view.frame.size.width, self.navigationController.toolbar.frame.size.height + 20)];
    topColor.backgroundColor = [UIColor colorWithRed:0.0 green:0.0 blue:0.0 alpha:0.8];
    [self.collectionView addSubview:topColor];
}
...
-(void) singleTapHandler:(ZQPhotoBrowserCell*) cell {
    ...
    if (self.needHiden) {
        ...
        [UIView animateWithDuration:UINavigationControllerHideShowBarDuration animations:^{
            ...
            [self.navigationController setNavigationBarHidden:YES animated:NO];//The navbar is automatically displayed
             if (self.toolbarItems) {
                  [self.navigationController setToolbarHidden:YES animated:NO];
             }
...
- (void) selectedIndex:(int)index{
    ...
    dispatch_after(dispatch_time(DISPATCH_TIME_NOW, (int64_t)(0.0 * NSEC_PER_SEC)), dispatch_get_main_queue(), ^{
        ...
        self.title = [NSString stringWithFormat:@"%zd/%zd",index+1,self.models.count];
    });
